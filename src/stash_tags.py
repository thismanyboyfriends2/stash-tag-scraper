"""Main tag transfer module using pure GraphQL."""
import logging
...
async def transfer_tags_graphql(
    ...
    if new_tags:
        ...
        if new_tags:
            ...
            log.info(f"Successfully created {len(created_ids)} new tags")
        else:
    ...
    if tags_to_update:
        ...
        with Progress(
            ...
            task = progress.add_task("Updating tags", total=len(tags_to_update))
            updated_count = await client.update_tags_batch(tags_to_update, progress=progress, task_id=task)

        if updated_count == len(tags_to_update):
            log.info(f"Successfully updated {updated_count} tags")
        else:
            log.warning(f"Updated {updated_count} of {len(tags_to_update)} tags (some updates failed)")
    else:
